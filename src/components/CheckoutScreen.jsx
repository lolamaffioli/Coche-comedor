...
import { ChevronLeft, MapPin, CreditCard, Banknote, Check } from "lucide-react";
...
export default function CheckoutScreen({
  ...
  return (
    <div className="flex flex-col h-full min-h-screen md:min-h-0 bg-card">
      {/* Header */}
      <div className="px-5 pt-12 pb-5" style={{ background: "linear-gradient(135deg, #091f41 0%, #1a5a9e 100%)" }}>
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="md:hidden w-9 h-9 rounded-full bg-white/15 flex items-center justify-center active:bg-white/25 transition-colors cursor-pointer">
        ...
        {delivery && (
          <div className="px-5 py-5 border-b border-border">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-3">
              {delivery === "seat" ? "Detalles del asiento" : "Horario de retiro"}
            </p>
              ...
              <div className="flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Ej: 3"
                      value={cocheNumber}
                      onChange={(e) => setCocheNumber(e.target.value.replace(/\D/g, ""))}
                      className="w-full bg-input-background border border-border rounded-xl px-4 py-3 text-foreground placeholder:text-muted-foreground outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all text-sm"
                      maxLength={3}
                    />
                  </div>
                  <div>
                ...
                <div className="bg-secondary rounded-2xl p-4 flex gap-4 items-start border border-border/50">
                  <div className="flex-shrink-0 w-16 h-20 bg-primary rounded-xl flex flex-col items-center justify-center gap-1 relative overflow-hidden">
                    ...
                    <div className="text-white text-xs font-bold z-10 truncate max-w-full px-1">
                      {cocheNumber ? `C${cocheNumber}` : "C?"} · {seatNumber || "A?"}
                    </div>
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-foreground mb-1">¿Dónde está el número?</p>
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      El coche y el asiento están impresos en tu pasaje impreso o digital de Trenes Argentinos.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {timeSlots.map((slot) => (
                  <button
                    ...
                    className={`py-2 rounded-xl text-xs font-medium border-2 transition-all active:scale-95 cursor-pointer text-center leading-tight ${
                      timeSlot === slot
        ...
        <div className="px-5 py-5">
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              className={`rounded-xl py-4 px-4 flex items-center gap-4 border-2 transition-all active:scale-[0.97] cursor-pointer ${
            ...
            {payment === "cash" && (
              <div className="bg-background border border-border rounded-2xl p-4 flex flex-col gap-3 transition-all">
                <p className="text-xs font-semibold text-foreground">¿Con cuánto vas a pagar?</p>
                <div className="flex gap-2 items-center">
                  <span className="text-sm font-bold text-primary">$</span>
                  <input
                    type="text"
                    placeholder={`Ej: ${totalPrice}`}
                    value={cashAmount}
                    onChange={(e) => setCashAmount(e.target.value.replace(/\D/g, ""))}
                    className="flex-1 bg-input-background border border-border rounded-xl px-3 py-2 text-foreground outline-none focus:border-accent text-sm"
                  />
                  <button
                    type="button"
                    onClick={() => setCashAmount(totalPrice.toString())}
                    className="text-xs font-semibold text-accent bg-accent/10 hover:bg-accent/20 px-3 py-2.5 rounded-xl transition-colors cursor-pointer"
                  >
                    Pago exacto
                  </button>
                </div>
            ...
            {payment !== null && (
              <div className="bg-secondary rounded-xl px-4 py-3 flex gap-3 items-start border border-border/50">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  El pago es <strong className="text-foreground">presencial</strong>. Nuestro personal completará la transacción al entregarte el pedido.
                </p>
              </div>
            )}
          </div>
      ...
      {/* Place order */}
      <div className="px-5 pb-8 pt-3 bg-background border-t border-border mt-auto">
        <button
          onClick={canCheckout ? onPlace : undefined}
          className={`w-full rounded-xl py-4 font-semibold text-sm transition-all cursor-pointer ${
            canCheckout
              ? "bg-primary text-white active:scale-[0.98]"
              : "bg-muted text-muted-foreground cursor-not-allowed"
          }`}
        >
          {canCheckout ? `Confirmar pedido · $${totalPrice.toLocaleString()}` : "Ingresá coche/asiento y método de pago"}
        </button>
      </div>
    </div>
  );
